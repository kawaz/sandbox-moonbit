...
- **try_reduce は投機的実行のため例外を投げない** — 内部で try? → catch → Reject。消費ループは全候補に投機的に try_reduce を呼ぶため、非採用候補のエラーで全体が落ちてはいけない
...
※ 最終設計は Parser struct + getter/setter 方式（後述）。ここでは概念説明のためシンプルな形を示す
...
Design rationale: 結果は getter/setter クロージャ内に隠蔽される（Parser struct + getter 方式で統一）。
...
**ctx.get() の stale read 特性**: 投機実行中は前 iteration の確定値のみが見える。未確定の候補の値は見えない。

Design rationale: 投機実行の安全性のため。全候補に並行して try_reduce を呼ぶモデルでは、ある候補の結果が別の候補から見えると非決定的な動作になる。確定値のみを公開することで、try_reduce の結果が呼び出し順序に依存しないことを保証する。
...
**値の所有権は getter/setter クロージャ内に隠蔽**（Parser struct + getter 方式で統一。後述のコア設計を参照）。
...
fn get[T](self : ResultMap, opt : Opt[T]) -> T {
  opt.getter(self.id)    // 直接 T を返す。ダウンキャスト不要
}
...
(2) **型ありビュー** — getter クロージャ経由で直接 T として取り出す
...
`result.get(port)` は内部的に `port.getter(result.id)` で直接 T を返す。
...
- 各 clone_fn が対応する Opt[T] の内部 Map に新 ID 対応の独立 `Ref[T]` を作成
- 雛形の Opt[T] をレンズとして使う際、`opt.getter(cloned_result.id)` で正しい値にアクセス
...
`try_reduce` の内部メカニズム:
- クロージャが Opt[T] の内部 `Ref[T]` をキャプチャしている
- `result_map.id` を使って正しい Ref を選択し、ユーザー定義 reducer を呼び出す
- ユーザー定義 reducer が raise した ParseError は内部で catch され、Reject として返される
- Accept の `consumed` は消費した引数の数を示す
- Accept の `result` は型消去された一時結果で、commit_reduce でのみ使用される

`commit_reduce` の内部メカニズム:
- try_reduce で得た ReduceResult を受け取り、ResultMap に書き込む
- 候補が1つに確定した後にのみ呼ばれる（先食い最長一致で勝者が決まった後）
...
- **force_positional**: `false` で初期化。`--` (double dash) が Accept された時に `true` に遷移
  - `true` の間、OC 候補から Option を除外し、Positional のみが候補になる
  - トリガー条件: name="" + greedy + global の Flag が Accept → force_positional 遷移

### アルゴリズム — 先食い最長一致モデル

消費ループは**先食い最長一致**で勝者を選ぶ。全候補に投機的に try_reduce を呼び、最も多くの引数を消費した候補を採用する。
...
2. カレントスコープから OC リスト（Option/Command）と P リスト（Positional）を収集
   - force_positional が true の場合、OC リストから Option を除外
   - コンテナ（Array/serial/or 等）なら再帰的に1つ目の OC を取得
   - フラットな OC リストにする
   - OC に引っかからなかったものは P リストへ

3. 投機実行: 全 OC 候補に対して try_reduce を呼ぶ（副作用なし）
...
5. Accept した候補が0:
   5a. args(n+1) なし → パース完了。消費ループ脱出
   5b. P リストなし → ParseError（予期しない引数）
   5c. P リストあり → P モードに遷移して消費ループ継続

6. Accept した候補のうち consumed が最大のものを選ぶ:
   6a. 最大 consumed が複数 → ParseError（曖昧）
   6b. 最大 consumed が1つ → 勝者確定

7. 勝者の commit_reduce を呼んで ResultMap に書き込み
   consumed 分だけ引数を shift
   カレントスコープ更新、消費ループ継続
   - 勝者が name="" + greedy + global の Flag の場合 → force_positional = true
```
...
- **P モードでもスコープ認識局所分解は適用される**（短縮オプション展開含む）

```
P モード:
  token = args(n)
  スコープ認識局所分解を適用（--port=8080 分解、-abc 短縮展開）
...
Design rationale: 引数消費を OC モードと P モードの2層で処理することで、Option/Command を確実に先に解決し、Positional は残り物として扱う。P モードでも Option の割り込みを許容するため、`cmd file1 --verbose file2` のような自然な記述が可能。Command は P モード移行後は候補から外れるため、位置引数とサブコマンド名の衝突を防ぐ。先食い最長一致モデルにより、消費量の異なる候補が競合した場合に最も多くの引数を消費した候補が自然に勝つ。
...
- Accept 時に消費ループの状態変数 `force_positional = true` に遷移（前述のアルゴリズム参照）
...
Design rationale: 従来 `--` は tokenize 段階で `DoubleDash` として特殊トークン化し、パーサ内でハードコードされた特殊処理をしていた。greedy + serial の組み合わせで `--` を通常の Opt として表現することで、特殊処理が不要になり、`--exec` 的な「以降全て引数」パターンにも同じ仕組みで対応できる。force_positional 状態変数により、`--` 以降は Option を候補から除外する動作を実現する。
...
**概要**: Parser struct が ID 空間と ref ストレージを一元管理。Opt[T] は getter/setter クロージャで型消去を解決。
...
struct Opt[T] {
  ...
  setter : (Int, T) -> Unit  // instance_id を受け取り T を書き込む（ErasedNode の reduce_erased 経由ではなく直接書き込み用）
...
**メリット**:
- Opt[T] struct フィールドは全て不変（getter/setter クロージャの参照先が mutable なだけ）
- 値の格納は getter/setter クロージャ内に隠蔽される
- ref 管理が Parser に一元化（メモリリーク問題が構造的に解消）
...
- **did you mean? サジェスト** — Reduce フェーズ内の名前解決フックとして実装。Levenshtein 距離によるスペルミス候補提示
...
| **Reduce** | 消費ループ（値の畳み込み）+ 名前解決 | `(ReduceCtx[T]) -> T?!ParseError` |
...
名前解決は Reduce フェーズ内で各ノードの try_reduce が行う。各ノードが自分の名前マッチングを知っているため、独立した Resolve フェーズは不要。
...
### Resolve フック — 名前解決のカスタマイズ

名前解決は各ノードの try_reduce 内で行われるが、解決前にフックで介入可能。

組み込み: 完全一致 → プレフィックスマッチ → 反転パターン（`--no-`, `--enable-`, `--disable-`）
フック: did-you-mean サジェスト、あいまいマッチ、カスタム辞書

Resolve フックは Reduce フェーズ内の名前解決前に呼び出される。

### Reduce — 消費ループ

ReduceCtx[T] 方式（前述）。パーサのコアロジック。名前解決もこのフェーズ内で行う。
...
| `--` (double dash) | `flag(name="", greedy=true, global=true)` + serial + force_positional で統一表現 |
| OC モード | 初期モード。Option/Command 優先、Positional はフォールバック |
| P モード | OC 全滅後。Option 割り込み可（スコープ認識局所分解含む）、Command 不可 |
...
src/
  ...
  parse/      # 消費ループ（Reduce フェーズのコアロジック）+ スコープ認識局所分解 + 名前解決
...
- `Opt[T]` — id, initial, reducer, meta, getter, setter（PoC4 の Parser struct + getter 方式）
...
#### Step 3: スコープ認識局所分解 + 消費ループ + 名前解決（parse/）

パーサのコアロジック。Reduce フェーズの実体。名前解決もこの中で行う。
...
**名前解決**（旧 Resolve フェーズの責務を統合）:

- 各ノードの try_reduce 内で名前解決を行う
- 各ノードが自分の名前マッチングを知っている（「名前解決自体が、opt/command をたくさん持ってて、次の引数にマッチする opt をその中から探す、という機能を持った Reducer」）
- ロングオプション完全一致
- エイリアスマッチ
- プレフィックスマッチ（曖昧でなければ）
- 反転パターン（`--no-xxx`, `--enable-xxx`, `--disable-xxx`）
- Resolve フックは try_reduce 内の名前解決前に呼び出される

**消費ループ（先食い最長一致）**:

- OC/P モード切り替え（OC: Option/Command 優先、P: Positional 優先）
- force_positional 状態管理（`--` による Option 除外）
- 全候補に投機的に try_reduce を呼び、consumed 最大の候補を選択
- 勝者の commit_reduce で ResultMap に書き込み
- サブコマンドマッチ時のスコープ切り替え
...
- **テスト**: フラグ/値オプション/位置パラメータの基本パース、サブコマンド切り替え、OC/P モード遷移、`--` 処理、先食い最長一致の候補選択、名前解決（完全一致、エイリアス、プレフィックス、反転パターン、曖昧時エラー）、未知オプションエラー
...
- `parser.add_resolve_hook(fn)` — Resolve フック登録（Reduce フェーズ内の名前解決をカスタマイズ）
- `parser.add_constraint(fn)` — Validate フック登録
- `parser.add_finalizer(fn)` — Finalize フック登録
- フックチェーンの実行順序管理（登録順）
- `parser.parse()` 内部をフックパイプライン駆動に再構成（Step 3 のロジックを Reduce フックとして再配置）
- **依存**: Step 3
...
`src/` はフラットにせず、パッケージ分割で管理する（`core/`, `parse/`, `validate/`, `help/`, `complete/` 等）。MoonBit のテストファイル命名パターン（`foo_test.mbt`, `foo_wbtest.mbt` 等）により、機能ごとのファイル数が増加するため。具体的な分割粒度は実装進行に合わせて決定。参考: mizchi の MoonBit リポジトリ群のパッケージ分割構成。
